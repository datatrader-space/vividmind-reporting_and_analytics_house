...
from django.contrib import admin
from .models import  Task, TaskAnalysisReport, JobAnalysisReport, CostUnitConfig,TaskReport
...
# reporting_and_analytics/admin.py
from django.contrib import admin
from .models import TaskAnalysisReport, Task,TaskSummaryReportNew # Ensure Task is imported if not already

@admin.register(TaskReport)
class TaskReportAdmin(admin.ModelAdmin):
    list_display = (
        'id', 'task', 'run_id', 'service', 'data_point',
        'report_start_datetime', 'report_end_datetime', 'created_at'
    )
    list_filter = ('service', 'data_point', 'created_at')
    search_fields = ('run_id', 'service', 'data_point')
    ordering = ('-created_at',)

    readonly_fields = ('created_at',)

@admin.register(TaskSummaryReportNew)
class TaskSummaryReportNewAdmin(admin.ModelAdmin):
    # 1. Display Fields in List View
    list_display = (
        'task_id_short',
        'total_reports_considered',
        'total_critical_events',
        'total_users_scraped',  # ✅ New
        'total_downloaded_files',  # ✅ New
        'failed_to_download_file_count',  # ✅ New
        'storage_upload_failed',  # ✅ New
        'has_billing_exception',  # ✅ New
        'latest_task_status',
        'latest_login_status',
        'has_next_page_info',
        'updated_at',
    ...
    # 4. Clickable Fields in List View
    list_display_links = (
        'task_id_short',
    )

    # 5. Detail View Field Grouping
    fieldsets = (
        (None, {
            'fields': ('task', 'task_id_display'),
        }),
        ('Report Time Range', {
            'fields': (
                'total_reports_considered',
        ...
        ('Failed Attempts & Errors', {
            'fields': (
                'total_attempt_failed',
                'attempt_failed_errors',
                'failed_attempt_error_logs',
            ),
        }),
        ('Scrape Summary', {  # ✅ New Section
            'fields': (
                'total_users_scraped',
                'total_downloaded_files',
                'total_storage_uploads',
                'failed_to_download_file_count',
        ...
        ('Exception Summaries', {
            ...
            'fields': (
                ...
                'login_exceptions_count',
                'page_detection_exceptions_summary',
                'page_detection_exceptions_count',
                'locate_element_exceptions_summary',
                'locate_element_exceptions_count',
            ),
        }),
        ('Page Load Info', {
            'classes': ('collapse',),
    ...
    # 6. Make JSON/Meta Fields Readonly
    readonly_fields = [
        'task', 'task_id_display',
        
        'total_login_attempts', 'successful_logins', 'failed_logins', 'total_login_time',
        'total_2fa_attempts', 'total_2fa_successes', 'total_2fa_failures', 'total_2fa_time',
         'attempt_failed_errors', 'failed_attempt_error_logs',
        'login_exceptions_summary', 'login_exceptions_count',
        'page_detection_exceptions_summary', 'page_detection_exceptions_count',
        'locate_element_exceptions_summary', 'locate_element_exceptions_count',
        'page_load_details',
        'total_reports_considered', 'first_report_datetime', 'last_report_datetime',
        'latest_task_status', 'latest_login_status',
        'latest_report_start_datetime', 'latest_report_end_datetime',
        'latest_total_task_runtime', 'run_id_of_latest_report',
        'has_next_page_info', 'updated_at', 'last_alerted_at',

        # ✅ New readonly scrape fields
        'total_users_scraped',
        'total_downloaded_files',
        'total_storage_uploads',
        'failed_to_download_file_count',
    ...
    # 7. Custom Short Display for Task ID
    def task_id_short(self, obj):
        return str(obj.task.uuid)[:8] + '...' if obj.task else 'N/A'
    task_id_short.short_description = 'Task ID'
    task_id_short.admin_order_field = 'task__uuid'

    # 8. Full Task ID in detail view
    def task_id_display(self, obj):
        return str(obj.task.uuid) if obj.task else 'N/A'
    task_id_display.short_description = 'Full Task ID'
